...
setGlobals () {
	...
	if [ $ORG -eq 1 ] ; then
		...
		CORE_PEER_TLS_ROOTCERT_FILE=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/org3.healthcare.com/peers/peer0.org3.healthcare.com/tls/ca.crt
		...
		if [ $PEER -eq 0 ]; then
			CORE_PEER_ADDRESS=peer0.org3.healthcare.com:7051
		else
			CORE_PEER_ADDRESS=peer1.org3.healthcare.com:7051
		fi
	else
...
installChaincode () {
	...
	VERSION=${3:-1.0}
        set -x
	peer chaincode install -c ${CHANNEL_NAME} -n HRecords-contract -v ${VERSION} -l ${LANGUAGE} -p ${CC_SRC_PATH} >&log.txt
	res=$?
        set +x
	cat log.txt
...
instantiateChaincode () {
	...
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
                set -x
		peer chaincode instantiate -o orderer.healthcare.com:7050 -C $CHANNEL_NAME -n HRecords-contract -l ${LANGUAGE} -v ${VERSION}  -P "OR	('HProviderMSP.peer','ResearchMSP.peer')" >&log.txt
		res=$?
                set +x
	else
                set -x
		peer chaincode instantiate -o orderer.healthcare.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n HRecords-contract -l ${LANGUAGE} -v 1.0  -P "OR	('HProviderMSP.peer','ResearchMSP.peer')" >&log.txt
		res=$?
                set +x
	fi
	cat log.txt
...
upgradeChaincode () {
    ...
    set -x
    peer chaincode upgrade -o orderer.healthcare.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n HRecords-contract -v 2.0 -P "OR ('HProviderMSP.peer','ResearchMSP.peer','Org3MSP.peer')"
    res=$?
	set +x
    cat log.txt
...
chaincodeQuery () {
  ...
  do
     ...
     set -x
     peer chaincode query -C $CHANNEL_NAME -n HRecords-contract -c '{"Args":["query","a"]}' >&log.txt
	 res=$?
     set +x
...
chaincodeInvoke () {
	...
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
                set -x
		peer chaincode invoke -o orderer.healthcare.com:7050 -C $CHANNEL_NAME -n HRecords-contract -c '{"Args":["createHealthcareLabs", "1003", "1001","1001","erikson","Labs Records","1217212121"]}' >&log.txt
		res=$?
                set +x
	else
                set -x
		peer chaincode invoke -o orderer.healthcare.com:7050  --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n HRecords-contract -c '{"Args":["createHealthcareLabs", "1003", "1001","1001","erikson","Labs Records","1217212121"]}' >&log.txt
		res=$?
                set +x
	fi
	cat log.txt
