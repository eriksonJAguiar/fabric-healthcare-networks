...
installChaincode() {
  ...
  VERSION=${3:-1.0}
  set -x
  peer chaincode install -n hrecords -v ${VERSION} -l ${LANGUAGE} -p ${CC_SRC_PATH} >&log.txt
  res=$?
  set +x
  cat log.txt
...
instantiateChaincode() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer chaincode instantiate -o orderer.healthcare.com:7050 -C $CHANNEL_NAME -n hrecords -l ${LANGUAGE} -v ${VERSION} -c '{"Args":[]}' -P "AND ('HProviderMSP.peer','ResearchMSP.peer')" >&log.txt
    res=$?
    set +x
  else
    set -x
    peer chaincode instantiate -o orderer.healthcare.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n hrecords -l ${LANGUAGE} -v 1.0 -c '{"Args":[]}' -P "AND ('HProviderMSP.peer','ResearchMSP.peer')" >&log.txt
    res=$?
    set +x
  fi
  cat log.txt
...
upgradeChaincode() {
  ...
  set -x
  peer chaincode upgrade -o orderer.healthcare.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n hrecords -v 2.0 -c '{"Args":["init","a","90","b","210"]}' -P "AND ('HProviderMSP.peer','ResearchMSP.peer','Org3MSP.peer')"
  res=$?
  set +x
  cat log.txt
...
chaincodeQuery() {
  ...
  do
    ...
    set -x
    peer chaincode query -C $CHANNEL_NAME -n hrecords -c '{"Args":["query","a"]}' >&log.txt
    res=$?
    set +x
...
chaincodeInvoke() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer chaincode invoke -o orderer.healthcare.com:7050 -C $CHANNEL_NAME -n hrecords $PEER_CONN_PARMS -c '{"Args":["invoke","a","b","10"]}' >&log.txt
    res=$?
    set +x
  else
    set -x
    peer chaincode invoke -o orderer.healthcare.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n hrecords $PEER_CONN_PARMS -c '{"Args":["invoke","a","b","10"]}' >&log.txt
    res=$?
    set +x
  fi
  cat log.txt
