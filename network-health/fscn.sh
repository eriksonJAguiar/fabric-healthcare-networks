...
export COMPOSE_PROJECT_NAME="healthsharing"
...
COMPOSE_FILE=./compose-files/docker-compose-node1.yml
LANGUAGE=node
...
if [ "$MODE" == "up" ]; then
  ...
  EXPMODE="Generating certs and genesis block for Health Chain Network"
else
  printHelp
  exit 1
fi
